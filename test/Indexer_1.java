 ...
 * @author Qpa
 */
public class Indexer_1 extends java.util.TimerTask
{
  /**
  ...
  private class EntryStripper extends org.pdfbox.util.PDFTextStripper
  {
     ...
     */
    public EntryStripper(org.pdfbox.pdmodel.PDPage pdPage, java.lang.String entry, java.util.Vector<java.awt.Rectangle.Float> boundingRectangles) throws java.io.IOException
    {
      super();
      this.pdPage = pdPage;
      this.entry = entry;
    ...
    protected void showCharacter(org.pdfbox.util.TextPosition text)
    {
      ...
      java.lang.String textToken = "";
      java.lang.String entryToken = "";
      if(matchCase)
      {
        textToken = text.getCharacter();
        entryToken = entry;
      }
      else
      {
      ...
      if(matchWord)
      {
        ...
        java.lang.String entryMatcher = ".*" + notWordCaracter + entryToken + notWordCaracter + ".*";
        if(textToken.matches(entryMatcher))
        {
          if(findAll)
          {
            while(textToken.matches(entryMatcher))
            {
              java.awt.Rectangle.Float boundingRectangle = getBounds(text, textX, textCharacter, textToken, entryToken);
              boundingRectangles.add(boundingRectangle);
              textX = boundingRectangle.x + boundingRectangle.width;
              int index = textToken.indexOf(entryToken) + entryToken.length();
              textCharacter = textCharacter.substring(index);
              textToken = textToken.substring(index);
            }
          }
          else if(matchCount == 0)
          {
            boundingRectangles.add(getBounds(text, textX, textCharacter, textToken, entryToken));
            matchCount ++;
          }
        }
      }
      ...
      {
        if(textToken.contains(entryToken))
        {
          if(findAll)
          {
            while(textToken.contains(entryToken))
            {
              java.awt.Rectangle.Float boundingRectangle = getBounds(text, textX, textCharacter, textToken, entryToken);
              boundingRectangles.add(boundingRectangle);
              textX = boundingRectangle.x + boundingRectangle.width;
     ...
     * Overridden method from <code>org.pdfbox.util.PDFTextStripper</code>.
     */
    public void processStream(org.pdfbox.pdmodel.PDPage pdPage, org.pdfbox.pdmodel.PDResources pdResources, org.pdfbox.cos.COSStream cosStream)  throws java.io.IOException
    {
      super.processStream(pdPage, pdResources, cosStream);
//System.out.println(pageCharacter);
  ...
  /** Shows if the first match or all matches has to be retreaved per one page. */
  ...
  /** The visualising text area or null of there is no. */
//  private javax.swing.JTextArea textArea;
   ...
   */
  public Indexer_1(java.net.URL pdfURL, java.lang.String password, java.util.Vector<java.lang.String> entries,
    boolean matchCase, boolean matchWord, boolean findAll, java.util.Vector<Interval> intervals,
  ...
  public void run()
  {
    ...
    try
    {
      final java.lang.String beginText = "<html><head><style type=\"text/css\">p {margin-top: 1}</style></head><body>";
      java.lang.String bodyText = "";
      final java.lang.String endText = "</body></html>";
      ...
      for(java.lang.String iteratorEntry : entries)
      {
        ...
        for(Interval iteratorInterval : intervals)
        {
          for(int page = iteratorInterval.getFromPage(); page <= iteratorInterval.getToPage() ; page++ )
          {
            ...
            EntryStripper entryStripper = new EntryStripper(pdPage, iteratorEntry, boundingRectangles);
            entryStripper.processStream(pdPage, pdPage.findResources(), pdPage.getContents().getStream());
        ...
        Index index = new Index(iteratorEntry, destinations);
        indexes.add(index);
        bodyText += index.toHTMLLine();
        final java.lang.String actualBodytext = bodyText;
        if(editorPane != null)
        {
          com.pdfindexer.util.SwingWorker editorWorker = new com.pdfindexer.util.SwingWorker()
          {
            ...
            {
              editorPane.setText(beginText + actualBodytext + endText);
              editorPane.scrollRectToVisible(new java.awt.Rectangle(0, editorPane.getHeight(), 0 , 1));
              return null;
            }
          };
